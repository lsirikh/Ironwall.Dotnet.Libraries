...
public sealed class LabelOffsetCommand : UndoableCommandBase
{
    ...
    public LabelOffsetCommand(IUndoApplyContext ctx, int id, (double x, double y) before, (double x, double y) after) : base(ctx)
    ...
    {
        var m = Ctx.FindMarkerById(_id); if (m == null) return;
...
public sealed class LockCommand : UndoableCommandBase
{
    ...
    public LockCommand(IUndoApplyContext ctx, int id, bool before, bool after) : base(ctx)
    { _id = id; _before = before; _after = after; }
...
public sealed class RenameSymbolCommand : UndoableCommandBase
{
    ...
    public RenameSymbolCommand(IUndoApplyContext ctx, int id, string before, string after) : base(ctx)
    { _id = id; _before = before ?? string.Empty; _after = after ?? string.Empty; }
