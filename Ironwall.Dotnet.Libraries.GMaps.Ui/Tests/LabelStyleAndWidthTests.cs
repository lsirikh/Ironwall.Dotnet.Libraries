...
public class LabelStyleAndWidthTests
{
    ...
    [Fact(DisplayName = "기본값 삼위일치: 스타일 기본 = White/Black(EnumColorType, FillColor 파이프라인)·200px (v2.5)")]
    public void should_keep_legacy_visual_defaults_when_style_fields_untouched()
    {
        var s = new SymbolModel();
        Assert.Equal(Ironwall.Dotnet.Libraries.Enums.EnumColorType.White, s.TitleColor);
        Assert.Equal(Ironwall.Dotnet.Libraries.Enums.EnumColorType.Black, s.TitleBackground);
        ...
        Assert.Equal(Ironwall.Dotnet.Libraries.Enums.EnumColorType.White, im.TitleColor);
        Assert.Equal(Ironwall.Dotnet.Libraries.Enums.EnumColorType.Black, im.TitleBackground);
    ...
    public void should_delegate_title_fields_to_model_when_image_marker_set()
    {
        ...
        marker.TitleColor = Ironwall.Dotnet.Libraries.Enums.EnumColorType.Cyan;
        Assert.Equal(Ironwall.Dotnet.Libraries.Enums.EnumColorType.Cyan, model.TitleColor);
    ...
    public void should_copy_all_title_fields_when_image_model_cloned()
    {
        var src = new ImageModel
        {
            ...
            TitleColor = Ironwall.Dotnet.Libraries.Enums.EnumColorType.Lime,
            TitleBackground = Ironwall.Dotnet.Libraries.Enums.EnumColorType.Transparent,
        ...
        Assert.Equal(Ironwall.Dotnet.Libraries.Enums.EnumColorType.Lime, dst.TitleColor);
        Assert.Equal(Ironwall.Dotnet.Libraries.Enums.EnumColorType.Transparent, dst.TitleBackground);
    ...
    [Fact(DisplayName = "BrushFromColorType: Transparent=투명, 배경은 ChipAlpha 합성(Black→#CD212121≈종전 칩) (v2.5)")]
    public void should_build_brush_from_color_type()
    {
        Assert.Same(System.Windows.Media.Brushes.Transparent,
            LabelAdorner.BrushFromColorType(Ironwall.Dotnet.Libraries.Enums.EnumColorType.Transparent, LabelAdorner.ChipAlpha));
        var bg = (System.Windows.Media.SolidColorBrush)LabelAdorner.BrushFromColorType(
            Ironwall.Dotnet.Libraries.Enums.EnumColorType.Black, LabelAdorner.ChipAlpha);
        Assert.Equal(LabelAdorner.ChipAlpha, bg.Color.A);   // 칩 알파 유지(종전 0xCD)
        Assert.Equal(0x21, bg.Color.R);                     // Black=#212121(Material Dark)
        Assert.True(bg.IsFrozen);
        var fg = (System.Windows.Media.SolidColorBrush)LabelAdorner.BrushFromColorType(
            Ironwall.Dotnet.Libraries.Enums.EnumColorType.White, null);
        Assert.Equal(0xFF, fg.Color.A);                     // 글자색은 불투명
