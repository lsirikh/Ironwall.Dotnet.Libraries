...
public static class ResponseHelper
{
    ...
    /// <summary>
    /// HttpResponseMessage → ApiResponse<T> 변환 (로깅 없음)
    /// </summary>
    public static async Task<ApiResponse<T>> ToApiResponseAsync<T>(this HttpResponseMessage response)
    {
        return await ToApiResponseAsync<T>(response, null);
    }

    /// <summary>
    /// HttpResponseMessage → ApiResponse<T> 변환 (로깅 포함)
    /// </summary>
    public static async Task<ApiResponse<T>> ToApiResponseAsync<T>(this HttpResponseMessage response, ILogService? log)
    {
        try
        {
            ...
            if (log != null)
            {
                log.Info($"[GOP API Response] Status: {(int)response.StatusCode} {response.ReasonPhrase}");
                log.Info($"[GOP API Response] Raw JSON: {content}");
            }
    ...
    /// <summary>
    /// HttpResponseMessage → ApiListResponse<T> 변환 (로깅 없음)
    /// </summary>
    public static async Task<ApiListResponse<T>> ToApiListResponseAsync<T>(this HttpResponseMessage response)
    {
        return await ToApiListResponseAsync<T>(response, null);
    }

    /// <summary>
    /// HttpResponseMessage → ApiListResponse<T> 변환 (로깅 포함)
    /// </summary>
    public static async Task<ApiListResponse<T>> ToApiListResponseAsync<T>(this HttpResponseMessage response, ILogService? log)
    {
        try
        {
            var content = await response.Content.ReadAsStringAsync();
