...
public interface IStreamingContextPool
{
    ...
    void Return(StreamingContext context);
    void Clear();
}

public class StreamingContextPool : IStreamingContextPool, IDisposable
{
    private readonly ConcurrentBag<StreamingContext> _pool;
    ...
    {
        _maxSize = maxSize;
        _pool = new ConcurrentBag<StreamingContext>();

        // 초기 생성
        for (int i = 0; i < Math.Min(8, maxSize); i++)
        {
            _pool.Add(new StreamingContext());
        }
    }

    public StreamingContext Get()
    {
    ...
    public void Return(StreamingContext context)
    {
