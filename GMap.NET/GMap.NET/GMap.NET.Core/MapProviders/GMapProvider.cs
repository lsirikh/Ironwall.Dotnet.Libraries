...
namespace GMap.NET.MapProviders
{
    ...
    public abstract class GMapProvider
    {
        ...
        protected GMapProvider()
        {
            ...
            Debug.WriteLine($"[GMapProvider] 등록 시도: Id={Id}, DbId={DbId}, Name={Name}, 기존 Provider 수={MapProviders.Count}");

            // Id 충돌 시 예외 (동일 GUID는 허용 불가)
            var idConflict = MapProviders.Find(p => p.Id == Id);
            if (idConflict != null)
            {
                Debug.WriteLine($"[GMapProvider] Id 충돌! 기존={idConflict.Name}({idConflict.Id}), 신규={Name}({Id})");
                throw new Exception("such provider id already exists, try regenerate your provider guid...");
            }

            // DbId 충돌 시 재생성 (SHA1 해시 4바이트 → int 충돌 가능)
            var originalDbId = DbId;
            while (MapProviders.Exists(p => p.DbId == DbId))
            {
                Debug.WriteLine($"[GMapProvider] DbId 충돌: {DbId}, 재생성 시도");
                DbId = Math.Abs(DbId + 1);
            }
            if (DbId != originalDbId)
                Debug.WriteLine($"[GMapProvider] DbId 재생성: {originalDbId} → {DbId}");

            MapProviders.Add(this);
            Debug.WriteLine($"[GMapProvider] 등록 완료: {Name}, Id={Id}, DbId={DbId}");
        }

        /// <summary>
        /// Provider를 static MapProviders 리스트에서 제거 (CustomMap 비활성화 시 사용)
        /// </summary>
        public static void UnregisterProvider(GMapProvider provider)
        {
