...
namespace GMap.NET.MapProviders
{
    ...
    public class MBTilesMapProvider : GMapProvider
    {
        ...
        /// <summary>
        /// 빈 타일(해당 zoom/x/y에 실제 타일이 없을 때) 대신 반환할 기본 타일 PNG 바이트.
        /// WPF 레이어에서 1회 생성해 주입한다(Core는 WPF 비의존 → byte[]만 보관).
        /// null이면 기존 동작(빈 타일 = null 반환)을 그대로 유지한다.
        /// 물음표(byte[]?) 미사용: 본 Core 프로젝트는 Nullable 미활성(C# 8) → CS8632 회피.
        /// </summary>
        public byte[] DefaultTileBytes { get; set; }

        public override PureImage GetTileImage(GPoint pos, int zoom)
        {
            if (source == null) return null;                       // (a) 맵 미로드 → 유지(로드 실패 은폐 방지)
            if (zoom < MinZoom || zoom > MaxZoom) return null;      // (b) 줌 범위 밖 → 유지(부모타일 폴백 보존)

            var img = source.GetImage(pos, zoom);                  // (c) 정상 줌
            // 타일이 없을 때만 기본 타일 반환. 공유 인스턴스 금지 — GetTileImageFromArray로 매 요청
            // 새 PureImage 생성(타일 퇴출 시 Tile.Dispose가 Data.Dispose 호출 → use-after-dispose 방지).
            // 캐시 참고: 기본 타일은 메모리 캐시에는 들어가나, MBTiles는 AccessMode.ServerOnly라
            // 디스크 캐시에는 영구 저장되지 않는다. 맵 교체 시 SwitchMBTilesMap의 MemoryCache.Clear로
            // 메모리 캐시도 해소되어, 이후 실제 타일이 추가돼도 기본 타일이 고착되지 않는다.
        ...
        public bool Open(string MBTilesFilePath)
        {
            ...
            try
            {
                // 이전 source 명시적 해제 (SQLite 리소스 누수 방지)
                source?.Close();
                ...
                if (Bounds != null && CenterLocation == PointLatLng.Empty) CenterLocation = new PointLatLng((Bounds[0].Lat + Bounds[1].Lat) / 2.0, (Bounds[0].Lng + Bounds[1].Lng) / 2.0);
                if (CenterZoom < 0) CenterZoom = (int)(MinZoom + (MaxZoom - MinZoom) / 2);
