...
/****************************************************************************
   Purpose      :                                                          
   Created By   : GHLee                                                
   Created On   : 6/22/2025 6:49:00 PM                                                    
   Department   : SW Team                                                   
   Company      : Sensorway Co., Ltd.                                       
   Email        : lsirikh@naver.com                                         
****************************************************************************/
public class ActionEventPanelViewModel : BaseDataGridMultiPanelViewModel<ActionEventViewModel>
                                        , IHandle<CallDeleteActionEventProcessMessageModel>
{
    #region - Ctors -
    ...
    public override async void OnClickDeleteButton(object sender, RoutedEventArgs e)
    {
        ...
        await _eventAggregator.PublishOnCurrentThreadAsync(new OpenConfirmPopupMessageModel
        {
            ...
            MessageModel = new CallDeleteActionEventProcessMessageModel()
        });
    }
    ...
    public override async void OnClickSaveButton(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var insertList = ViewModelProvider
                             ...
                             .Select(vm => (IActionEventModel)vm.Model)
                             .ToList();
            ...
            {
                var model = (IActionEventModel)vm.Model;
                try { await _providerService.UpdateActionEventAsync(model, token); vm.IsEdited = false; }
            ...
            {
                if (model.OriginEvent == null || model.OriginEvent.Id <= 0) { held++; continue; }   // 원본이벤트 미지정 Draft 보류(서버 from_event_id FK 필수)
                try { var created = await _providerService.InsertActionEventAsync(model, token); if (created != null && created.Id > 0) model.Id = created.Id; }
                catch (OperationCanceledException) { throw; }
                catch (Exception ex)
                {
    ...
    public override void OnSelectionChanged(IList<ActionEventViewModel> rows)
    {
        SelectedItems = rows;
    ...
    private void CollectionEntity_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        switch (e.Action)
        {
            case NotifyCollectionChangedAction.Add:
                ...
                foreach (IActionEventViewModel newItem in e.NewItems)
                {
                    EventProvider.Add(newItem.Model);
                }
                break;
            ...
            case NotifyCollectionChangedAction.Remove:
                ...
                foreach (IActionEventViewModel oldItem in e.OldItems)
                {
                    EventProvider.Remove(oldItem.Model);
                }
                break;
            ...
            case NotifyCollectionChangedAction.Reset:
                ...
                foreach (var item in EventProvider.OfType<IActionEventModel>())
                {
    ...
    private Task DataInitialize(CancellationToken cancellationToken = default)
    {
        return Task.Run(async () =>
        {
            try
            {
                ...
                PagedResult<IActionEventModel> firstPage;
                try
                {
                    firstPage = await _providerService.FetchActionEventsPageAsync(
                ...
                catch (Exception ex)
                {
                    ...
                    firstPage = new PagedResult<IActionEventModel> { Success = false };
                }
                ...
                if (!firstPage.Success)
                {
                    await _eventAggregator.PublishOnUIThreadAsync(new OpenInfoPopupMessageModel
                    {
                        Explain = "조치 이벤트를 불러오지 못했습니다. 서버 연결 상태를 확인하세요. 기존 목록은 유지됩니다."
                    }, cancellationToken);
    ...
    private IList<ActionEventViewModel> _pendingDeleteItems = [];
    #endregion
}